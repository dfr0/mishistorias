...
for i in $(cat $list); do echo "####$i  $timestamp####";echo $CLAVE | ./logica.sh $i |awk '{print $5}';sshpass -p $PWD ssh -t -o ConnectTimeout=5 -o StrictHostKeyChecking=no $USER@$i "$execute" ;done | tee -a $log
}
